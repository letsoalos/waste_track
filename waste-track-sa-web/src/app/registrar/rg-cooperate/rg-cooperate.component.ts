...
@Component({
  selector: 'app-rg-cooperate',
  templateUrl: './rg-cooperate.component.html',
  styleUrls: ['./rg-cooperate.component.scss']
})
export class RgCooperateComponent implements OnInit{
  ...
  loadWastePickerList() {
    this.registrarService.getWastePickers().subscribe({
      next: (res: WastePickerDto[]) => {
        this.rowData = res.filter((picker: WastePickerDto) => picker.isMemberCooperative === true);
      },
      error: (err) => {
        console.log(err);
     }
  ...
  onCellClicked(params: any) {
    if (params.column.colId === 'action' && params.event.target.dataset.action) {
      ...
      if (action === 'view') {
        this.router.navigate(['registrar/cooperate/', params.data.id]);
      }
    }
  }
